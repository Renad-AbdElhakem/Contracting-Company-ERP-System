...
{
    public class AddStockToCompanyWarehouseCommandHandler : IRequestHandler<AddStockToCompanyWarehouseCommand, GeneralResponse<int>>
    {
        ...
        public AddStockToCompanyWarehouseCommandHandler(
          IMapper mapper,
            ICompanyWarehouseRepository warehouseRepository,
        ...
        public async Task<GeneralResponse<int>> Handle(AddStockToCompanyWarehouseCommand request, CancellationToken cancellationToken)
        {
            var warehouse = await _warehouseRepository.GetByIdAsync(request.warehouseId);

            if (warehouse == null)
                return GeneralResponse<int>.Fail($"Company warehouse with id {request.warehouseId} not found");
            ...
            var stock = _mapper.Map<CompanyWarehouseStock>(request.StockDto);
          
            stock.ArrivalDate = DateOnly.FromDateTime(DateTime.Now);
          
            warehouse.CompanyWarehouseStocks.Add(stock);
